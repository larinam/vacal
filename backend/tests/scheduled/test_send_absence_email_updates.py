...
def test_send_absence_email_updates_skips_unsubscribed_topics():
    tenant = Tenant(
        name=f"Tenant{uuid.uuid4()}",
        identifier=str(uuid.uuid4()),
    ).save()
    ...
    subscriber_full = User(
        name="Alice",
        email=f"alice{uuid.uuid4()}@example.com",
        tenants=[tenant],
        auth_details=AuthDetails(username=str(uuid.uuid4())),
    ).save()
    subscriber_other = User(
        name="Bob",
    ...
    Team(
        ...
        notification_subscriptions=[
            TeamNotificationSubscription(
                user=subscriber_full,
                topics=[topic.value for topic in NotificationTopic.defaults()],
            ),
            TeamNotificationSubscription(
                user=subscriber_other,
                topics=[NotificationTopic.UPCOMING_ABSENCES.value],
            ),
        ],
    ).save()
    ...
    with patch("backend.scheduled.absence_starts.send_email") as mock_send_email, \
        ...
        sent_to = [args[2] for args, _ in mock_send_email.call_args_list]
        assert subscriber_full.email in sent_to
        assert subscriber_other.email not in sent_to
